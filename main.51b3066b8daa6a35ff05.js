...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  var boxes = gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.utils.toArray(".galery-slider__slide");
  ...
  function horizontalLoop(items, config) {
    items = gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.utils.toArray(items);
    config = config || {};
    var tl = gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.timeline({
      ...
      snap = config.snap === false ? function (v) {
        return v;
      } : gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.utils.snap(config.snap || 1),
      ...
      item,
      i;
    gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.set(items, {
      // convert "x" to "xPercent" to make things responsive, and populate the widths/xPercents Arrays to make lookups faster.
      xPercent: function xPercent(i, el) {
        var w = widths[i] = parseFloat(gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.getProperty(el, "width", "px"));
        xPercents[i] = snap(parseFloat(gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.getProperty(el, "x", "px")) / w * 100 + gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.getProperty(el, "xPercent"));
        return xPercents[i];
      }
    });
    gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.set(items, {
    ...
    totalWidth = items[length - 1].offsetLeft + xPercents[length - 1] / 100 * widths[length - 1] - startX + items[length - 1].offsetWidth * gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.getProperty(items[length - 1], "scaleX") + (parseFloat(config.paddingRight) || 0);
    for (i = 0; i < length; i++) {
      item = items[i];
      curX = xPercents[i] / 100 * widths[i];
      distanceToStart = item.offsetLeft + curX - startX;
      distanceToLoop = distanceToStart + widths[i] * gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.getProperty(item, "scaleX");
      tl.to(item, {
    ...
    function toIndex(index, vars) {
      ...
      var newIndex = gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.utils.wrap(0, length, index),
        time = times[newIndex];
      if (time > tl.time() !== index > curIndex) {
        // if we're wrapping the timeline's playhead, make the proper adjustments
        vars.modifiers = {
          time: gsap__WEBPACK_IMPORTED_MODULE_6__.gsap.utils.wrap(0, tl.duration())
        };
...
/******/ })()
;
//# sourceMappingURL=main.51b3066b8daa6a35ff05.js.map